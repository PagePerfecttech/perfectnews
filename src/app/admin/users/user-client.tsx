...
import { UserPlus, Shield, Mail, Trash2, Edit, UserCircle } from 'lucide-react';
import { updateUserRole, deleteUser } from '@/lib/user-actions';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
         ...
         <button disabled={isPending} className="bg-primary text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-red-100 hover:scale-105 transition-all flex items-center space-x-2 disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {[
           { label: 'Total Staff', count: totalStaff, icon: UserCircle, color: 'text-blue-600' },
           { label: 'Active Editors', count: totalEditors, icon: Shield, color: 'text-primary' },
           { label: 'Field Reporters', count: totalReporters, icon: Mail, color: 'text-green-600' },
         ].map((stat, i) => (
                    ...
                    <tr key={user.id} className="group hover:bg-gray-50 transition-colors">
                       <td className="px-8 py-6">
                          <div className="flex items-center space-x-4">
                             ...
                             <div>
                                <p className="font-black italic uppercase tracking-tighter text-secondary">{user.name || 'Anonymous'}</p>
                       ...
                       <td className="px-8 py-6">
                          <span className="px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest bg-green-50 text-green-600">
                             ACTIVE
                          </span>
                       </td>
                       <td className="px-8 py-6 text-right">
                          <div className="flex items-center justify-end space-x-2">
                             ...
                             <button onClick={() => handleDelete(user.id)} disabled={isPending} className="p-2 hover:bg-white hover:shadow-md rounded-lg text-gray-400 hover:text-primary transition-all disabled:opacity-50">
