...
export function BroadcastClient({ editions }: { editions: { title: string, id: string, date: string }[] }) {
  const [copied, setCopied] = useState(false);
  const [selectedId, setSelectedId] = useState(editions[0]?.id || "");
  
  const selectedEdition = editions.find(e => e.id === selectedId) || editions[0];

  const waMessage = selectedEdition 
    ? `🌞 *శుభోదయం! నేటి ఈ-పేపర్ మీ కోసం...*\n\n📰 *${selectedEdition.title}* - ${selectedEdition.date}\n\nనేటి ముఖ్యాంశాలు మరియు పూర్తి వార్తల కోసం ఈ క్రింది లింక్ క్లిక్ చేయండి 👇\n\n🔗 https://telugupost.com/epaper/${selectedEdition.id}\n\n--- \nTelugu Post - అక్షర రూపం, ప్రజా పక్షం!`
    : `🌞 *శుభోదయం! నేటి ఈ-పేపర్ మీ కోసం...*\n\n📰 *Hyderabad Edition*\n\nనేటి ముఖ్యాంశాలు మరియు పూర్తి వార్తల కోసం ఈ క్రింది లింక్ క్లిక్ చేయండి 👇\n\n🔗 https://telugupost.com/epaper\n\n--- \nTelugu Post - అక్షర రూపం, ప్రజా పక్షం!`;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           ...
           {selectedEdition && (
             <div className="flex items-center space-x-3 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
                ...
                <span className="text-xs font-black uppercase tracking-widest">{selectedEdition.date}</span>
             </div>
           )}
        </div>
          ...
          <div className="lg:col-span-7 space-y-8">
             <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-8">
                <div className="flex items-center space-x-4 border-b border-gray-50 pb-6">
                   ...
                   <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-bold uppercase">Choose which paper to blast</p>
                   </div>
                </div>

                <div className="space-y-2">
                   <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest flex items-center">
                     <MapPin className="w-3 h-3 mr-1" /> Published E-Papers
                   </label>
                   {editions.length > 0 ? (
                     <select 
                       value={selectedId}
                     ...
                     <div className="p-4 bg-red-50 text-red-500 rounded-2xl text-xs font-bold border border-red-100">
                       No E-Papers found in database. Please upload an E-Paper first.
                     </div>
                   )}
                </div>
                ...
                <div className="bg-primary/5 p-6 rounded-3xl border border-primary/10 flex items-start space-x-4">
                   ...
                   <div>
                      ...
                      <p className="text-xs text-gray-500">The front page of this edition will be attached as the link preview image.</p>
                   </div>
                </div>
             </section>
             ...
             <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  className="flex items-center justify-center space-x-3 bg-white border-2 border-gray-100 p-5 rounded-3xl font-black text-xs uppercase tracking-widest hover:border-primary transition-all disabled:opacity-50"
                >
                ...
                <button 
                  disabled={!selectedEdition}
                  onClick={shareToWA}
                  className="flex items-center justify-center space-x-3 bg-primary text-white p-5 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl shadow-red-900/20 hover:scale-105 transition-all disabled:opacity-50"
                >
                      ...
                      <div className="bg-gray-100 rounded-lg overflow-hidden border border-gray-200 mb-3">
                         ...
                         {selectedEdition && (
                           <div className="p-3 bg-gray-50 border-t border-gray-100">
                              <p className="text-[10px] font-black text-secondary">{selectedEdition.title.toUpperCase()}</p>
                              <p className="text-[8px] text-gray-400 font-bold">{selectedEdition.date}</p>
                           </div>
                         )}
                      </div>
